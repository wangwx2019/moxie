# -*- coding: utf-8 -*-
"""
完整修复脚本：为一年级下册所有生字提供适龄组词和造句
- 组词：简单、常见、适合一年级
- 造句：简单、自然、逻辑通顺
"""

import sys, json, re
sys.stdout.reconfigure(encoding='utf-8')

# 完整数据库：每个字的2个组词 + 1个造句
# 格式: '字': ('词语1', '词语2', '造句')
word_db = {
    # ===== 第一单元 =====
    # 识字1：春夏秋冬
    '霜': ('霜雪', '冰霜', '早上草地上有白白的霜。'),
    '吹': ('吹风', '吹牛', '春天吹来的风很暖和。'),
    '落': ('落下', '落叶', '秋天到了，树叶落下来。'),
    '降': ('降落', '下降', '气球慢慢降落下来。'),
    '飘': ('飘扬', '飘动', '红旗在空中飘扬。'),
    '游': ('游泳', '游戏', '夏天我喜欢去游泳。'),
    '池': ('水池', '池塘', '公园里有一个大池塘。'),
    '入': ('入口', '出入', '我们从入口进入公园。'),
    '春': ('春天', '春风', '春天到了，花儿都开了。'),
    '冬': ('冬天', '冬日', '冬天会下雪，很冷。'),
    '花': ('花朵', '红花', '公园里开满了美丽的花朵。'),
    '飞': ('飞机', '飞走', '小鸟飞走了。'),

    # 识字2：姓氏歌
    '氏': ('姓氏', '氏族', '老师教我们认识姓氏。'),
    '什': ('什么', '为什么', '你在做什么呢？'),
    '李': ('李子', '姓李', '我爱吃甜甜的李子。'),
    '张': ('张开', '姓张', '他张开双手欢迎我们。'),
    '弓': ('弓箭', '拉弓', '他会拉弓射箭。'),
    '古': ('古代', '古人', '古代的人穿不一样的衣服。'),
    '胡': ('胡子', '胡说', '爷爷留着长长的胡子。'),
    '吴': ('吴国', '姓吴', '我们班有一个同学姓吴。'),
    '言': ('语言', '言论', '我们要学好自己的语言。'),
    '孙': ('孙子', '子孙', '爷爷的孙子很可爱。'),
    '么': ('多么', '那么', '今天的天气多么好啊！'),
    '双': ('双手', '一双', '我有一双小手。'),

    # 识字3：小青蛙
    '河': ('河水', '小河', '小河里有好多小鱼。'),
    '晴': ('晴天', '晴朗', '今天是个大晴天。'),
    '眼': ('眼睛', '眼泪', '我们要保护好自己的眼睛。'),
    '睛': ('眼睛', '定睛', '眼睛是心灵的窗户。'),
    '保': ('保护', '保卫', '我们要保护小树。'),
    '护': ('护士', '爱护', '护士阿姨很辛苦。'),
    '苗': ('禾苗', '树苗', '田里长出了绿绿的禾苗。'),
    '吃': ('吃饭', '小吃', '我们要按时吃饭。'),
    '事': ('事情', '故事', '妈妈给我讲了一个故事。'),
    '情': ('心情', '事情', '我今天的心情很好。'),
    '请': ('请坐', '请求', '老师请我们坐下。'),
    '让': ('让开', '让座', '公交车上我给老奶奶让座。'),
    '青': ('青草', '青山', '山上长满了青草。'),
    '清': ('清水', '清楚', '小河的水很清。'),

    # 识字4：猜字谜
    '猜': ('猜谜', '猜想', '我们来猜一个谜语吧。'),
    '边': ('旁边', '一边', '桌子旁边有一把椅子。'),
    '凉': ('凉快', '凉水', '秋天天气凉快了。'),
    '喜': ('喜欢', '喜爱', '我很喜欢画画。'),
    '欢': ('欢乐', '欢迎', '我们一起欢乐地唱歌。'),
    '时': ('时间', '时候', '现在是学习时间。'),
    '怕': ('害怕', '不怕', '我很害怕打雷。'),
    '攻': ('攻击', '进攻', '战士们勇敢地进攻。'),
    '令': ('命令', '口令', '老师发出了口令。'),
    '感': ('感情', '感谢', '我对妈妈很感激。'),
    '动': ('运动', '动手', '我们一起做运动吧。'),
    '万': ('千万', '一万', '花园里有千万朵花。'),
    '无': ('无声', '无边', '天空万里无云。'),
    '字': ('写字', '汉字', '我会写很多汉字。'),
    '红': ('红色', '红花', '花儿红红的，很好看。'),
    '明': ('明天', '明白', '明天我要去公园。'),

    # 语文园地一
    '识': ('认识', '知识', '我们要认识很多字。'),
    '组': ('小组', '组长', '我们分成几个小组。'),
    '计': ('计算', '计划', '老师教我们学计算。'),
    '算': ('计算', '算数', '我会做数学计算。'),
    '减': ('减少', '减法', '五减二等于三。'),
    '式': ('公式', '方式', '黑板上写着数学公式。'),
    '图': ('图画', '地图', '我画了一幅美丽的图画。'),
    '形': ('形状', '圆形', '这个形状是圆形。'),
    '卡': ('卡片', '卡车', '我有一张好看的书卡。'),
    '合': ('合作', '合上', '大家一起合作玩游戏。'),
    '唱': ('唱歌', '合唱', '我们一起来唱歌。'),
    '团': ('团结', '团长', '我们要团结友爱。'),
    '文': ('文字', '文化', '我们要好好学习文化。'),
    '片': ('卡片', '一片', '天空中飘着一片云。'),

    # ===== 第二单元 =====
    # 阅读1：热爱中国共产党
    '热': ('热闹', '热水', '夏天天气很热。'),
    '爱': ('爱心', '爱护', '我们爱自己的妈妈。'),
    '共': ('共同', '一共', '我们共有六个小组。'),
    '产': ('生产', '产品', '工人叔叔生产很多东西。'),
    '党': ('党员', '党旗', '我们从小热爱共产党。'),
    '太': ('太阳', '太大', '今天的太阳太大了。'),
    '阳': ('太阳', '阳光', '早晨的阳光很温暖。'),
    '光': ('灯光', '光明', '教室里的灯光很亮。'),
    '怀': ('怀抱', '怀念', '妈妈的怀抱很温暖。'),
    '抱': ('拥抱', '抱着', '妈妈紧紧地抱着我。'),
    '幸': ('幸福', '幸运', '我们过得很幸福。'),
    '福': ('幸福', '福气', '我们要珍惜幸福生活。'),
    '成': ('成功', '成为', '我长大了想成为老师。'),

    # 阅读2：吃水不忘挖井人
    '井': ('水井', '井口', '村子里有一口老水井。'),
    '城': ('城市', '长城', '星期天我去城市玩。'),
    '村': ('农村', '村庄', '我的老家在一个小农村。'),
    '毛': ('毛笔', '羽毛', '我用毛笔写字。'),
    '主': ('主人', '主要', '妈妈是我家的主人。'),
    '席': ('主席', '出席', '毛主席是我们敬爱的人。'),
    '住': ('住房', '居住', '我们住在城市里。'),
    '乡': ('乡村', '家乡', '我的家乡很美丽。'),
    '亲': ('亲人', '亲爱', '我的亲人对我很好。'),
    '战': ('战士', '战斗', '战士们保卫祖国。'),
    '士': ('士兵', '士气', '解放军叔叔是勇敢的士兵。'),
    '想': ('想法', '想念', '我很想念我的奶奶。'),
    '念': ('想念', '念头', '我常常念叨家乡的事。'),
    '江': ('江水', '长江', '长江是中国最长的江。'),
    '方': ('方向', '地方', '早晨太阳从东方升起。'),
    '后': ('后面', '以后', '以后我要好好学习。'),

    # 阅读3：我多想去看看
    '告': ('告诉', '广告', '他告诉我一个好消息。'),
    '走': ('走路', '走开', '放学了，我们一起走路回家。'),
    '京': ('北京', '京城', '北京是祖国的首都。'),
    '座': ('座位', '一座', '公园里有一座大石桥。'),
    '安': ('安全', '安静', '我们要注意安全。'),
    '广': ('广大', '广场', '广场上有很多人在跳舞。'),
    '场': ('场地', '场合', '操场是我们做运动的地方。'),
    '非': ('非常', '是非', '今天天气非常好。'),
    '宽': ('宽度', '宽敞', '马路很宽，车很多。'),
    '丽': ('美丽', '华丽', '春天的花园很美丽。'),
    '洁': ('清洁', '洁白', '我们要保持教室清洁。'),
    '的': ('好的', '我的', '这是我的书包。'),
    '会': ('开会', '学会', '我学会了骑自行车。'),
    '北': ('北方', '北京', '冬天北方会下雪。'),

    # 语文园地二
    '认': ('认识', '认真', '我认识很多汉字。'),
    '连': ('连接', '连续', '大家连成一条长队。'),
    '选': ('选择', '选手', '我们选他当班长。'),
    '圈': ('圆圈', '圈子', '小狗在院子里跑了一圈。'),
    '涂': ('涂抹', '涂色', '我在纸上涂了红色。'),
    '填': ('填空', '填写', '请把这张表填写完整。'),
    '试': ('考试', '尝试', '明天我们要考试。'),
    '练': ('练习', '练笔', '我每天都要练习写字。'),
    '写': ('写字', '写作', '我要好好学习写字。'),

    # ===== 第三单元 =====
    # 阅读4：小公鸡和小鸭子
    '玩': ('玩具', '玩耍', '我们一起玩玩具吧。'),
    '得': ('得到', '记得', '我得到了老师的表扬。'),
    '急': ('着急', '急忙', '要迟到了，我很着急。'),
    '直': ('直接', '直线', '公路笔直地伸向远方。'),
    '哭': ('哭泣', '哭声', '小妹妹哭了。'),
    '跟': ('跟着', '跟前', '小狗跟着我跑。'),
    '忽': ('忽然', '忽视', '天空忽然下起了雨。'),
    '然': ('然后', '自然', '放学后我先做作业，然后去玩。'),
    '听': ('听见', '听话', '我认真听老师讲课。'),
    '喊': ('喊叫', '大喊', '他大声喊我的名字。'),
    '快': ('快乐', '飞快', '我飞快地跑向学校。'),
    '己': ('自己', '知己', '自己的事情要自己做。'),
    '背': ('背书', '背后', '我会背很多古诗。'),
    '从': ('从前', '从来', '从前有一只可爱的小兔子。'),
    '好': ('好的', '好人', '我有一个好妈妈。'),
    '们': ('我们', '他们', '我们都是好孩子。'),
    '他': ('他们', '他的', '他是我的好朋友。'),
    '回': ('回家', '回来', '下午我要回家。'),
    '说': ('说话', '小说', '我们一起说话吧。'),
    '自': ('自己', '自由', '我们要自己做作业。'),

    # 阅读5：树和喜鹊
    '只': ('只有', '只要', '我只有一支笔。'),
    '很': ('很多', '很好', '我的书很多。'),
    '孤': ('孤单', '孤独', '我一个人很孤单。'),
    '单': ('简单', '单位', '这道题很简单。'),
    '种': ('种子', '种地', '春天我把种子种在地里。'),
    '每': ('每天', '每次', '我每天走路去上学。'),
    '都': ('都是', '都有', '同学们都到齐了。'),
    '邻': ('邻居', '邻近', '我的邻居很友好。'),
    '居': ('居住', '居民', '我们居住在这个小区。'),
    '叫': ('叫喊', '叫声', '小鸟在树上叫。'),
    '招': ('招呼', '招聘', '早上要主动打招呼。'),
    '呼': ('呼气', '呼吸', '我们一起深呼吸。'),
    '乐': ('快乐', '音乐', '我们一起快乐地唱歌。'),

    # 阅读6：怎么都快乐
    '怎': ('怎么', '怎样', '你怎么迟到了？'),
    '独': ('单独', '独立', '我单独一个人在家。'),
    '跳': ('跳舞', '跳高', '下课了，我们一起跳绳。'),
    '绳': ('绳子', '跳绳', '我有一条长绳子。'),
    '当': ('当然', '当时', '我当上了三好学生。'),
    '还': ('还有', '还是', '我还有很多作业没有写。'),
    '羽': ('羽毛', '羽毛球', '羽毛球是我喜欢的运动。'),
    '球': ('足球', '球类', '下课了我们一起踢足球。'),
    '劲': ('使劲', '干劲', '大家一起使劲推车。'),
    '轮': ('车轮', '一轮', '汽车有四个车轮。'),
    '排': ('排队', '排球', '我们排队去食堂吃饭。'),
    '书': ('书本', '读书', '我要好好读书。'),
    '画': ('画画', '图画', '我喜欢画画。'),
    '毛': ('毛笔', '羽毛', '我用毛笔写字。'),

    # 语文园地三
    '母': ('母亲', '父母', '母亲节我要送给妈妈礼物。'),
    '页': ('页码', '一页', '这本书有一百页。'),
    '止': ('停止', '禁止', '十字路口红灯表示停止。'),
    '斤': ('公斤', '斤两', '我买了两斤苹果。'),
    '寸': ('尺寸', '一寸', '一寸光阴一寸金。'),
    '丁': ('园丁', '丁香', '花园里开满了丁香花。'),
    '千': ('千万', '一千', '千山万水都走过。'),
    '全': ('全部', '完全', '今天同学全部到齐了。'),
    '元': ('一元', '元旦', '这个笔记本一元钱。'),

    # ===== 第四单元 =====
    # 阅读7：静夜思
    '静': ('安静', '静止', '教室里很安静。'),
    '思': ('思考', '思念', '我想念远方的亲人。'),
    '床': ('床铺', '床上', '我躺在床上睡觉。'),
    '疑': ('疑问', '怀疑', '他有疑问就去问老师。'),
    '举': ('举手', '举行', '我举手回答老师的问题。'),
    '望': ('希望', '远望', '我远远望着大山。'),
    '低': ('低头', '高低', '我低头捡起一支笔。'),
    '故': ('故事', '故乡', '外婆给我讲过去的故事。'),
    '前': ('前面', '从前', '我坐在教室前面。'),
    '地': ('地上', '地方', '地上长满了青草。'),
    '乡': ('乡村', '家乡', '我的家乡很美丽。'),

    # 阅读8：夜色
    '胆': ('胆子', '大胆', '弟弟胆子很小。'),
    '敢': ('勇敢', '敢于', '他敢一个人睡觉。'),
    '勇': ('勇敢', '勇气', '解放军叔叔很勇敢。'),
    '讲': ('讲话', '讲课', '老师讲故事很好听。'),
    '窗': ('窗户', '窗口', '我打开窗户看外面。'),
    '乱': ('乱跑', '混乱', '房间里很乱，要收拾一下。'),
    '拉': ('拉手', '拉住', '我拉着妈妈的手过马路。'),
    '样': ('样子', '一样', '他俩长得很像的样子。'),
    '笑': ('笑容', '笑话', '听到好笑的话大家都笑了。'),
    '再': ('再见', '再次', '明年我们再见吧。'),
    '睡': ('睡觉', '睡眠', '晚上要早点睡觉。'),
    '觉': ('觉得', '睡觉', '我觉得这本书很好看。'),
    '色': ('颜色', '红色', '我最喜欢红色。'),
    '把': ('把手', '一把', '我把书放进了书包。'),

    # 阅读9：端午粽
    '端': ('端午', '端正', '端午节的粽子真好吃。'),
    '粽': ('粽子', '粽叶', '妈妈包了很多粽子。'),
    '节': ('节日', '节目', '六月一日是儿童节。'),
    '总': ('总是', '总共', '妈妈总是帮我检查作业。'),
    '煮': ('煮饭', '煮面', '妈妈在厨房煮饭。'),
    '盼': ('盼望', '期盼', '我盼望过年穿新衣服。'),
    '米': ('大米', '米饭', '米饭是我们的主食。'),
    '枣': ('红枣', '枣子', '红枣营养很丰富。'),
    '甜': ('甜味', '甜美', '这个西瓜很甜。'),
    '分': ('分开', '分数', '我们分小组做游戏。'),
    '鲜': ('新鲜', '鲜花', '菜场里有新鲜的蔬菜。'),
    '肉': ('牛肉', '猪肉', '妈妈买了牛肉回来。'),
    '了': ('好了', '完了', '我长大了。'),
    '间': ('时间', '中间', '我在两个朋友中间。'),
    '吃': ('吃饭', '小吃', '我们要按时吃饭。'),

    # 语文园地四
    '册': ('画册', '手册', '我有一本好看的画册。'),
    '支': ('一支', '支持', '我有一支红色的笔。'),
    '台': ('台风', '台上', '讲台上有粉笔和黑板擦。'),
    '电': ('电话', '电脑', '家用电器给我们带来方便。'),
    '视': ('电视', '视力', '吃完晚饭我们一起看电视。'),
    '部': ('部分', '部门', '这本书有好几个部分。'),
    '机': ('飞机', '手机', '爸爸给我买了一架飞机模型。'),
    '衣': ('衣服', '雨衣', '妈妈给我买了新衣服。'),
    '裤': ('裤子', '短裤', '这条裤子有点长。'),
    '被': ('被子', '被动', '冬天要盖厚被子。'),

    # ===== 第五单元 =====
    # 识字5：动物儿歌
    '物': ('动物', '植物', '动物园里有很多动物。'),
    '捉': ('捉住', '捉迷藏', '小猫捉住了老鼠。'),
    '迷': ('迷路', '着迷', '我迷路了，找不到家。'),
    '藏': ('捉迷藏', '收藏', '我和同学玩捉迷藏。'),
    '造': ('造句', '制造', '小鸟在树上造房子。'),
    '蚂': ('蚂蚁', '蚂蚱', '地上爬着一只小蚂蚁。'),
    '蚁': ('蚂蚁', '白蚁', '蚂蚁在搬食物。'),
    '运': ('运动', '运气', '大家一起运书到教室。'),
    '食': ('食物', '粮食', '我们不能浪费食物。'),
    '粮': ('粮食', '粮仓', '小松鼠在储备粮食。'),
    '房': ('房子', '房间', '我的房间很整洁。'),
    '结': ('结果', '结束', '这个苹果结得很大。'),
    '网': ('上网', '渔网', '蜘蛛在树枝间结网。'),
    '造': ('造句', '制造', '小鸟在树上造房子。'),
    '欢': ('欢乐', '欢迎', '我们一起欢乐地唱歌。'),

    # 识字6：古对今
    '圆': ('圆形', '团圆', '十五的月亮圆圆的。'),
    '严': ('严格', '严重', '我们要严格要求自己。'),
    '寒': ('寒冷', '寒冬', '冬天天气很寒冷。'),
    '酷': ('酷热', '酷暑', '夏天天气酷热。'),
    '暑': ('暑假', '暑期', '暑假我去奶奶家玩。'),
    '暖': ('暖和', '温暖', '春天天气变暖和了。'),
    '晨': ('早晨', '清晨', '早晨的空气很新鲜。'),
    '细': ('细心', '仔细', '我写字很细心。'),
    '朝': ('朝阳', '朝霞', '早晨的朝阳真美丽。'),
    '霞': ('朝霞', '晚霞', '天边出现了美丽的晚霞。'),
    '夕': ('夕阳', '夕烟', '夕阳真美啊！'),
    '杨': ('杨树', '白杨', '路边种着一排白杨树。'),
    '香': ('香味', '香气', '花园里飘来阵阵香味。'),
    '对': ('对手', '不对', '这个说法不对。'),
    '雪': ('下雪', '雪花', '冬天会下雪。'),
    '语': ('语言', '语文', '我们要好好学习语文。'),

    # 识字7：操场上
    '操': ('操场', '操作', '下课了我们一起去操场。'),
    '拔': ('拔河', '选拔', '体育课上我们比赛拔河。'),
    '拍': ('拍手', '拍照', '我学会了拍皮球。'),
    '跑': ('跑步', '奔跑', '我每天早晨都跑步。'),
    '踢': ('踢球', '踢腿', '下课了我们一起踢足球。'),
    '铃': ('铃声', '铃铛', '上课铃声响了。'),
    '真': ('真的', '认真', '这个电影真好看。'),
    '闹': ('热闹', '闹钟', '市场里真热闹。'),
    '丢': ('丢脸', '丢弃', '我不小心丢了一支笔。'),
    '沙': ('沙子', '沙滩', '沙滩上有很多漂亮的贝壳。'),
    '身': ('身体', '身上', '我们要锻炼身体。'),
    '体': ('体育', '身体', '体育课我们要好好上。'),
    '打': ('打球', '打电话', '我爱打乒乓球。'),
    '皮': ('皮肤', '皮球', '我有一个新皮球。'),
    '足': ('足球', '手足', '足球是我们喜欢的运动。'),
    '包': ('书包', '包子', '我背着书包去上学。'),

    # 识字8：人之初
    '之': ('之前', '之后', '放学之后我要去公园。'),
    '初': ('初期', '最初', '这是最初的想法。'),
    '相': ('相同', '互相', '这两幅画很相似。'),
    '近': ('远近', '附近', '我家离学校很近。'),
    '习': ('学习', '练习', '我要好好学习。'),
    '远': ('远方', '远处', '远方有一座高山。'),
    '教': ('教书', '教室', '老师在教室里教我们。'),
    '道': ('道路', '道理', '这是一条宽宽的道路。'),
    '专': ('专心', '专业', '他做事很专心。'),
    '幼': ('幼儿', '幼儿园', '弟弟还在上幼儿园。'),
    '玉': ('玉石', '玉米', '这块玉石真漂亮。'),
    '知': ('知道', '知识', '我知道这个问题的答案。'),
    '义': ('正义', '意义', '我们要做一个有正义的人。'),
    '学': ('学生', '学习', '我是一名小学生。'),
    '义': ('正义', '意义', '我们要做一个有正义的人。'),

    # 语文园地五
    '饭': ('吃饭', '米饭', '中午我在学校吃饭。'),
    '饱': ('吃饱', '饱和', '午饭吃饱了才有力气。'),
    '茶': ('茶叶', '喝茶', '爷爷喜欢喝茶。'),
    '泡': ('泡泡', '泡茶', '我帮妈妈泡茶。'),
    '轻': ('轻轻', '年轻', '妈妈轻轻地关上门。'),
    '穿': ('穿衣', '穿过', '我穿上了新衣服。'),
    '袍': ('旗袍', '长袍', '舞台上的演员穿着长袍。'),
    '鞭': ('鞭子', '鞭炮', '过年要放鞭炮。'),
    '炮': ('大炮', '鞭炮', '鞭炮声真响啊。'),
    '饱': ('吃饱', '饱和', '午饭吃饱了才有力气。'),
    '抱': ('拥抱', '抱着', '妈妈紧紧地抱着我。'),

    # ===== 第六单元 =====
    # 阅读10：古诗二首
    '诗': ('诗歌', '诗人', '我会背很多古诗。'),
    '首': ('首先', '首长', '这是一首好听的歌。'),
    '偷': ('偷偷', '小偷', '我们不可以偷偷拿别人的东西。'),
    '浮': ('浮动', '漂浮', '小球浮在水面上。'),
    '萍': ('浮萍', '萍水', '池塘里长满了浮萍。'),
    '泉': ('泉水', '温泉', '山里有清澈的泉水。'),
    '惜': ('珍惜', '可惜', '我们要珍惜时间。'),
    '照': ('照亮', '照片', '阳光照在草地上。'),
    '柔': ('柔软', '温柔', '妈妈的头发很柔软。'),
    '荷': ('荷花', '荷叶', '池塘里开满了荷花。'),
    '露': ('露水', '露出', '清晨叶子上挂满了露水。'),
    '角': ('牛角', '角度', '牛有两只尖尖的角。'),
    '采': ('采集', '采花', '我去花园里采花。'),
    '尖': ('尖刀', '笔尖', '猫有尖尖的爪子。'),
    '早': ('早上', '早晨', '今天我早早起床了。'),

    # 阅读11：浪花
    '浪': ('浪花', '海浪', '海浪拍打着沙滩。'),
    '迈': ('迈步', '年迈', '我迈开大步向前走。'),
    '悄': ('悄悄', '静悄悄', '他悄悄走进了教室。'),
    '泪': ('眼泪', '泪水', '我的眼泪流下来了。'),
    '次': ('次数', '每次', '这次考试我得了第一。'),
    '给': ('给你', '送给', '妈妈给我买了新书包。'),
    '壳': ('贝壳', '外壳', '海边有很多漂亮的贝壳。'),
    '虾': ('虾米', '龙虾', '虾是一种营养丰富的食物。'),
    '装': ('装扮', '服装', '妈妈帮我装好了书包。'),
    '像': ('好像', '画像', '月亮像一只弯弯的小船。'),
    '淘': ('淘气', '淘米', '弟弟有时候很淘气。'),
    '娃': ('娃娃', '女娃', '布娃娃是我的好朋友。'),
    '玩': ('玩具', '玩耍', '我们一起玩玩具吧。'),
    '眼': ('眼睛', '眼泪', '我们要保护好自己的眼睛。'),
    '泪': ('眼泪', '泪水', '我的眼泪流下来了。'),
    '它': ('它们', '它的', '小狗在睡觉，它很可爱。'),
    '贝': ('贝壳', '宝贝', '海边有很多贝壳。'),
    '气': ('生气', '气体', '我生气了，不理他。'),

    # 阅读12：荷叶圆圆
    '珠': ('珠子', '珍珠', '珍珠闪闪发光。'),
    '摇': ('摇头', '摇晃', '风吹得树枝摇来摇去。'),
    '篮': ('篮子', '篮球', '妈妈提着一篮水果。'),
    '亮': ('明亮', '月亮', '天上的星星亮晶晶。'),
    '晶': ('水晶', '晶莹', '清晨的露水晶莹剔透。'),
    '停': ('停止', '停车', '红灯亮了，汽车停了下来。'),
    '坪': ('草坪', '坪坝', '草地上长满了绿草。'),
    '展': ('展开', '展示', '小鸟展开了翅膀。'),
    '透': ('透明', '透气', '玻璃透明透亮的。'),
    '翅': ('翅膀', '鱼翅', '小鸟的翅膀真有力。'),
    '膀': ('翅膀', '肩膀', '鸡的翅膀拍来拍去。'),
    '朵': ('花朵', '云朵', '树上开满了花朵。'),
    '机': ('飞机', '手机', '飞机在天空中飞翔。'),
    '台': ('台风', '台上', '讲台上有粉笔和黑板擦。'),
    '唱': ('唱歌', '合唱', '我们一起来唱歌。'),
    '伞': ('雨伞', '打伞', '下雨了，我撑开了雨伞。'),
    '美': ('美丽', '美好', '我们的校园真美丽。'),

    # 阅读13：要下雨了
    '要': ('要求', '需要', '老师要求我们要认真。'),
    '腰': ('弯腰', '腰部', '我弯腰捡起地上的纸。'),
    '阴': ('阴天', '阴暗', '今天天气阴沉沉的。'),
    '沉': ('沉重', '下沉', '石头沉到了水底。'),
    '呀': ('好呀', '哎呀', '这里的风景真美呀！'),
    '忙': ('帮忙', '忙碌', '妈妈忙着做家务。'),
    '呢': ('什么呢', '好吗', '你在干什么呢？'),
    '吗': ('好吗', '对吗', '你吃饭了吗？'),
    '面': ('面条', '面包', '妈妈做了一碗面条。'),
    '空': ('天空', '空气', '天空中有小鸟在飞。'),
    '闷': ('闷热', '烦闷', '天气很闷热，要下雨了。'),
    '吧': ('好吧', '来吧', '我们一起去玩吧。'),
    '消': ('消失', '消息', '火渐渐熄灭消失了。'),
    '息': ('休息', '利息', '中午我们要休息一下。'),
    '这': ('这里', '这个', '这个地方真漂亮。'),
    '看': ('看见', '看书', '我看见了一只小鸟。'),
    '鱼': ('小鱼', '金鱼', '鱼缸里有一条小鱼。'),
    '问': ('问题', '提问', '我有问题要问老师。'),
    '加': ('加法', '加上', '二加三等于五。'),

    # 语文园地六
    '棍': ('棍子', '木棍', '爷爷给我做了一根小木棍。'),
    '豆': ('豆芽', '豆子', '豆芽是一种蔬菜。'),
    '汤': ('汤水', '喝汤', '妈妈做的汤很好喝。'),
    '蚊': ('蚊子', '蚊虫', '夏天蚊子很多。'),
    '扇': ('扇子', '扇风', '夏天要用扇子扇风。'),
    '椅': ('椅子', '桌椅', '我坐在椅子上写字。'),
    '牵': ('牵手', '牵牛', '我牵着妈妈的手过马路。'),
    '织': ('织布', '编织', '蜘蛛在织网。'),
    '斗': ('北斗星', '斗争', '北斗星挂在天空。'),
    '豆': ('豆芽', '豆子', '豆芽是一种蔬菜。'),
    '斗': ('北斗星', '斗争', '北斗星挂在天空。'),

    # ===== 第七单元 =====
    # 阅读14：文具的家
    '具': ('工具', '具体', '我的文具盒里有铅笔。'),
    '铅': ('铅笔', '铅球', '我用铅笔写字。'),
    '新': ('新年', '新书', '我有一本新课本。'),
    '平': ('平安', '平时', '路面很平。'),
    '盒': ('盒子', '饭盒', '我有一个漂亮的铅笔盒。'),
    '些': ('一些', '这些', '这些书都是我的。'),
    '此': ('此时', '此地', '此时此刻我在认真学习。'),
    '仔': ('仔细', '仔鸡', '我仔细地做作业。'),
    '检': ('检查', '检阅', '我要检查作业有没有错。'),
    '查': ('查看', '调查', '医生给病人检查身体。'),
    '所': ('所以', '所有', '这些都是我所喜欢的。'),
    '伙': ('伙伴', '合伙', '我和伙伴一起玩游戏。'),
    '伴': ('伙伴', '陪伴', '伙伴们一起上学。'),
    '笔': ('铅笔', '笔画', '我有一支新铅笔。'),
    '知': ('知道', '知识', '我知道这个问题的答案。'),
    '道': ('道路', '道理', '这是一条宽宽的道路。'),
    '放': ('放学', '放手', '放学了，我们一起回家。'),
    '安': ('安全', '安静', '我们要注意安全。'),

    # 阅读15：一分钟
    '钟': ('时钟', '分钟', '时钟滴答滴答地走。'),
    '迟': ('迟到', '推迟', '今天我迟到了。'),
    '背': ('背书', '背后', '我会背很多古诗。'),
    '灯': ('电灯', '灯光', '晚上要开电灯。'),
    '等': ('等待', '平等', '我们一起等待公交车。'),
    '啊': ('好啊', '是啊', '我们一起去玩啊！'),
    '决': ('决定', '决心', '我决定要好好学习。'),
    '定': ('一定', '定时', '我一定要考一百分。'),
    '已': ('已经', '已知', '我已经做完作业了。'),
    '经': ('已经', '经过', '我经过学校门口。'),
    '位': ('位置', '座位', '我找到了自己的座位。'),
    '表': ('表示', '表现', '我表示同意。'),
    '车': ('汽车', '火车', '我坐公共汽车上学。'),
    '站': ('站立', '车站', '我在车站等车。'),
    '课': ('上课', '课文', '上课要专心听讲。'),
    '坐': ('座位', '坐下', '我坐在座位上。'),
    '老': ('老师', '老人', '老师教我们学知识。'),
    '师': ('老师', '师长', '老师像妈妈一样关心我们。'),

    # 阅读16：动物王国开大会
    '虎': ('老虎', '虎头', '动物园里有大老虎。'),
    '熊': ('熊猫', '狗熊', '熊猫是我国的国宝。'),
    '通': ('通知', '通过', '老师通知我们开会。'),
    '注': ('注意', '注目', '我们要注意安全。'),
    '意': ('意思', '意见', '你的意思我明白了。'),
    '遍': ('一遍', '普遍', '我把课文读了一遍。'),
    '百': ('一百', '百花', '花园里有上百朵花。'),
    '为': ('因为', '为什么', '因为明天放假，我很高兴。'),
    '因': ('因为', '原因', '因为下雨，我们不出去玩了。'),
    '舌': ('舌头', '舌尖', '我们要保护好自己的舌头。'),
    '理': ('理由', '理解', '我有理由不参加。'),
    '呀': ('好呀', '哎呀', '这里的风景真美呀！'),
    '忘': ('忘记', '遗忘', '我忘记了带橡皮。'),
    '国': ('国家', '中国', '我们的祖国是中国。'),
    '都': ('都是', '都有', '同学们都到齐了。'),
    '点': ('点头', '一点', '老师点点头表示同意。'),
    '林': ('树林', '森林', '远处有一片森林。'),

    # 阅读17：小猴子下山
    '猴': ('猴子', '猴王', '猴子在树上跳来跳去。'),
    '块': ('一块', '块状', '我吃了一块西瓜。'),
    '兴': ('高兴', '兴趣', '收到礼物我很高兴。'),
    '掰': ('掰开', '掰手腕', '我把馒头掰开吃。'),
    '扛': ('扛着', '扛枪', '农民伯伯扛着锄头下地。'),
    '往': ('来往', '往事', '我往学校走去。'),
    '棵': ('一棵树', '棵数', '门前有一棵大树。'),
    '满': ('满意', '充满', '我对这次考试很满意。'),
    '扔': ('扔掉', '扔球', '我不小心扔掉了铅笔。'),
    '摘': ('摘花', '采摘', '我去花园里摘花。'),
    '捧': ('捧着', '吹捧', '我捧着一束鲜花。'),
    '追': ('追赶', '追求', '小狗在追赶小猫。'),
    '高': ('高兴', '高大', '长得高可以拿到高处的东西。'),
    '着': ('看着', '听着', '我看着黑板认真听讲。'),
    '往': ('来往', '往事', '我往学校走去。'),
    '瓜': ('西瓜', '瓜子', '夏天西瓜很甜。'),
    '兔': ('兔子', '白兔', '小白兔长着长耳朵。'),
    '进': ('进来', '进步', '我走进了教室。'),

    # 语文园地七
    '刷': ('刷牙', '刷子', '早上我要刷牙洗脸。'),
    '梳': ('梳子', '梳理', '妈妈帮我梳头发。'),
    '巾': ('毛巾', '头巾', '我有一条干净的毛巾。'),
    '皂': ('肥皂', '香皂', '我用肥皂洗手。'),
    '洗': ('洗手', '洗澡', '吃饭前要洗手。'),
    '澡': ('洗澡', '澡堂', '晚上我要洗澡。'),
    '脸': ('脸盆', '洗脸', '我要把脸洗干净。'),
    '盆': ('脸盆', '盆栽', '我用一个盆洗脸。'),

    # ===== 第八单元 =====
    # 阅读18：棉花姑娘
    '棉': ('棉花', '棉衣', '棉衣冬天穿很暖和。'),
    '姑': ('姑娘', '姑妈', '我的姑姑很漂亮。'),
    '娘': ('姑娘', '大娘', '小姑娘在跳舞。'),
    '病': ('生病', '病人', '生病了要去医院。'),
    '她': ('她们', '她的', '她是我的好朋友。'),
    '治': ('治病', '治疗', '医生给病人治病。'),
    '燕': ('燕子', '燕窝', '燕子飞到南方去了。'),
    '帮': ('帮助', '帮忙', '我帮妈妈做家务。'),
    '害': ('害怕', '害虫', '我很害怕打雷。'),
    '别': ('别人', '分别', '别人说话时要认真听。'),
    '干': ('干净', '干部', '我把桌子擦干净。'),
    '惊': ('惊奇', '惊讶', '听到这个消息我很惊奇。'),
    '奇': ('奇怪', '奇妙', '大自然有很多奇怪的现象。'),
    '空': ('天空', '空气', '天空中有小鸟在飞。'),
    '还': ('还有', '还是', '我还有很多作业没有写。'),
    '身': ('身体', '身上', '我们要锻炼身体。'),
    '星': ('星星', '星空', '天上有很多星星。'),
    '久': ('很久', '久远', '好久不见，我很想念你。'),

    # 阅读19：咕咚
    '咕': ('咕咚', '咕咕', '肚子咕咕叫，我饿了。'),
    '咚': ('咕咚', '咚咚', '咚咚锵，过年真热闹。'),
    '熟': ('熟悉', '成熟', '这个西瓜熟透了。'),
    '掉': ('掉下', '丢掉', '我的书掉在地上了。'),
    '湖': ('湖水', '湖泊', '湖面上有小船在划。'),
    '吓': ('吓人', '吓跑', '打雷声吓了我一跳。'),
    '啦': ('好啦', '哗啦', '我们胜利啦！'),
    '鹿': ('小鹿', '鹿角', '小鹿在森林里跑。'),
    '象': ('大象', '气象', '大象的鼻子很长。'),
    '野': ('野外', '野生', '野外有很多美丽的花。'),
    '拦': ('拦住', '拦截', '红灯亮了，汽车拦下来。'),
    '哪': ('哪里', '哪个', '你在哪里？'),
    '那': ('那里', '那个', '那边的风景很美。'),
    '领': ('领子', '领导', '老师带领我们去参观。'),
    '吓': ('吓人', '吓跑', '打雷声吓了我一跳。'),
    '为': ('因为', '为什么', '因为明天放假，我很高兴。'),
    '怕': ('害怕', '不怕', '我很害怕打雷。'),
    '家': ('回家', '家人', '我要回家了。'),
    '没': ('没有', '没事', '我没有迟到。'),
    '到': ('到达', '来到', '我到了学校。'),

    # 阅读20：小壁虎借尾巴
    '壁': ('墙壁', '壁纸', '墙壁上挂着一幅画。'),
    '借': ('借书', '借用', '我向同学借了一块橡皮。'),
    '咬': ('咬住', '咬牙', '小狗咬住了骨头。'),
    '难': ('困难', '难过', '这道题很难。'),
    '哪': ('哪里', '哪个', '你在哪里？'),
    '爬': ('爬山', '爬行', '我爬上了山顶。'),
    '您': ('您好', '您的', '老师您辛苦了。'),
    '拨': ('拨动', '拨打', '他拨通了电话号码。'),
    '赶': ('赶快', '赶路', '我赶快跑向教室。'),
    '摆': ('摆动', '摆放', '他把书摆得整整齐齐。'),
    '过': ('过去', '过来', '我走过了小桥。'),
    '孩': ('孩子', '小孩', '小孩子要听大人的话。'),
    '转': ('转身', '转动', '我转过身来。'),
    '向': ('方向', '向上', '我向着学校走去。'),
    '边': ('旁边', '一边', '桌子旁边有一把椅子。'),
    '行': ('行走', '行人', '行人要走人行道。'),
    '草': ('草地', '小草', '地上长满了青草。'),
    '赶': ('赶快', '赶路', '我赶快跑向教室。'),
    '过': ('过去', '过来', '我走过了小桥。'),
    '找': ('找到', '寻找', '我找到了丢失的铅笔。'),

    # 语文园地八
    '吵': ('吵架', '吵闹', '请不要大声吵架。'),
    '现': ('现在', '发现', '我发现了一朵漂亮的花。'),
    '顶': ('头顶', '顶点', '屋顶上有一只小猫。'),
    '胖': ('胖子', '肥胖', '吃太多会变得很胖。'),
    '票': ('车票', '门票', '妈妈给我买了电影票。'),
    '户': ('户口', '窗户', '家家户户都亮起了灯。'),
    '交': ('交给', '交通', '我把作业交给老师。'),
    '父': ('父亲', '父母', '父亲工作很辛苦。'),
    '页': ('页码', '一页', '这本书有一百页。'),
}

print('✅ 词汇数据库加载完成，共 %d 个字' % len(word_db))

# 读取原MD文件
md_path = r'C:\Users\Administrator\Desktop\moxie-main\一年级下册生字组词造句 - 副本.md'
with open(md_path, 'r', encoding='utf-8') as f:
    lines = f.readlines()

# 处理每一行
new_lines = []
fixed_count = 0
for line in lines:
    stripped = line.strip()
    # 检查是否是表格数据行
    if stripped.startswith('|') and not stripped.startswith('|------') and '生字' not in stripped and '----' not in stripped:
        parts = [p.strip() for p in stripped.split('|')]
        # parts: ['', '霜', '霜雪', '冰霜', '造句', '']
        if len(parts) >= 5:
            char = parts[1]
            old_word1 = parts[2]
            old_word2 = parts[3]
            old_sentence = parts[4]
            
            # 如果字在数据库中，替换组词和造句
            if char in word_db:
                word1, word2, sentence = word_db[char]
                new_line = '| {} | {} | {} | {} |\n'.format(char, word1, word2, sentence)
                new_lines.append(new_line)
                fixed_count += 1
            else:
                # 不在数据库中，保留原行
                new_lines.append(line)
        else:
            new_lines.append(line)
    else:
        new_lines.append(line)

# 写回文件
with open(md_path, 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print('✅ MD文件修复完成！共修复 %d 处' % fixed_count)
print('   文件路径:', md_path)

# ========== 生成JSON文件 ==========
print('\n📦 开始生成JSON文件...')

# 重新读取修复后的MD文件
with open(md_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 解析结构
result = {
    'title': '一年级下册 生字组词与造句',
    'units': []
}
current_unit = None
current_lesson = None
current_type = None  # 'shizi' or 'xiezhi'

lines = content.split('\n')
i = 0
while i < len(lines):
    line = lines[i].strip()
    
    # 匹配单元 ## 第一单元·识字
    if line.startswith('## '):
        if current_unit and current_lesson:
            current_unit['lessons'].append(current_lesson)
            current_lesson = None
        if current_unit:
            result['units'].append(current_unit)
        current_unit = {'name': line[3:].strip(), 'lessons': []}
    
    # 匹配课程 ### 识字1：春夏秋冬
    elif line.startswith('### '):
        if current_unit and current_lesson:
            current_unit['lessons'].append(current_lesson)
        current_lesson = {'name': line[4:].strip(), 'shizi': [], 'xiezhi': []}
        current_type = None
    
    # 匹配类型 #### 识字（会认）或 #### 写字（会写）
    elif line.startswith('#### '):
        type_name = line[5:].strip()
        if '识字' in type_name and '写' not in type_name:
            current_type = 'shizi'
        elif '写字' in type_name:
            current_type = 'xiezhi'
        else:
            current_type = None
    
    # 匹配表格行 | 霜 | 霜雪 | 冰霜 | 造句 |
    elif line.startswith('|') and not line.startswith('|------'):
        # 跳过表头
        if '生字' in line or '----' in line:
            i += 1
            continue
        
        # 解析表格行
        parts = [p.strip() for p in line.split('|')]
        # parts = ['', '霜', '霜雪', '冰霜', '造句', '']
        if len(parts) >= 5 and current_lesson and current_type:
            char = parts[1]
            word1 = parts[2]
            word2 = parts[3]
            sentence = parts[4]
            
            # 跳过空行或无效行
            if char and char != '生字' and len(char) == 1:
                entry = {
                    'char': char,
                    'word1': word1,
                    'word2': word2,
                    'sentence': sentence
                }
                current_lesson[current_type].append(entry)
    
    i += 1

# 追加最后一个lesson和unit
if current_unit and current_lesson:
    current_unit['lessons'].append(current_lesson)
if current_unit:
    result['units'].append(current_unit)

# 保存为JSON
output_path = r'C:\Users\Administrator\Desktop\moxie-main\一年级下册生字组词造句.json'
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print('✅ JSON文件生成完成！')
print('   文件路径:', output_path)
print('   单元数:', len(result['units']))

# 统计
total_shizi = 0
total_xiezhi = 0
for unit in result['units']:
    for lesson in unit['lessons']:
        total_shizi += len(lesson['shizi'])
        total_xiezhi += len(lesson['xiezhi'])

print('\n📊 统计信息:')
print('   识字表（会认）:', total_shizi, '字')
print('   写字表（会写）:', total_xiezhi, '字')
print('   总计:', total_shizi + total_xiezhi, '字')

# 显示前3个示例
print('\n📝 示例数据（前3个）:')
count = 0
for unit in result['units']:
    for lesson in unit['lessons']:
        for item in lesson['shizi'][:1] + lesson['xiezhi'][:1]:
            if count < 3:
                print('   {}: {}、{} → {}'.format(item['char'], item['word1'], item['word2'], item['sentence']))
                count += 1
    if count >= 3:
        break
